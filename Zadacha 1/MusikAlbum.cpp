...
void MusikAlbum::addSong(const Song& newSong)
{
...
double MusikAlbum::totallenght() const
{
    double total = 0.0;
    for(int i = 0; i<numberofSongs; i++)
    {
...
size_t MusikAlbum::countofSongsofGenre(const char* genre) const
{
    size_t countSongs = 0;
    if(genre == nullptr)
    {
...
size_t MusikAlbum::countofSongsofSinger(const char* singer) const
{
    size_t countSongs = 0;
    if(singer == nullptr)
    {
    ...
    for(int i = 0; i < numberofSongs; i++)
    {
        if(strcmp(songs[i].getSinger(), singer)==0)
        {
            countSongs++;
        }
    }
